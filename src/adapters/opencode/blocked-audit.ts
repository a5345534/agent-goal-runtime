...
interface TurnEvidence {
  signatures: string[];
  hasUpdateGoalCall: boolean;
  timestamp?: string;
}

export function buildOpencodeBlockedAuditEvidence(
  options: OpencodeBlockedAuditOptions,
  ...
  for (const message of options.messages) {
    ...
    if (message.role !== "assistant" && message.role !== "user" && message.role !== "toolResult") continue;
  ...
  for (let index = signatures.length - 1; index >= 0; index -= 1) {
    if (signatures[index] !== latest) break;
    consecutive += 1;
  }
...
function signatureFromToolResult(message: OpencodeMessage): string | undefined {
...
function textFromMessageContent(message: OpencodeMessage): string {
  const chunks: string[] = [];
  for (const part of message.parts ?? []) {
...
// Re-export for callers that want to derive an `unused` `goal` reference.
