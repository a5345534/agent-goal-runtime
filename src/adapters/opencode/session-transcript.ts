...
export interface OpencodeReadOptions {
  client: { session?: { messages?: (params: { sessionID: string }) => Promise<{ data?: OpencodeMessage[]; error?: unknown }> } };
  sessionID: string;
}
...
export async function readOpencodeSessionMessages(options: OpencodeReadOptions): Promise<OpencodeMessage[]> {
  if (!options.client.session?.messages) return [];
  const response = await options.client.session.messages({ sessionID: options.sessionID });
  if (response.error) return [];
  return Array.isArray(response.data) ? response.data : [];
}

export function summariseOpencodeSession(messages: OpencodeMessage[]): OpencodeTranscriptSnapshot {
  const snapshot: OpencodeTranscriptSnapshot = {
    messages,
    hasError: false,
    hasAborted: false,
  ...
  for (const message of messages) {
    ...
    if (message?.stopReason === "aborted") snapshot.hasAborted = true;
    if (message?.stopReason === "error" || message?.errorMessage) snapshot.hasError = true;
...
export function readOpencodeTokenUsage(messages: OpencodeMessage[]): TokenUsageSnapshot {
  let input = 0;
  let output = 0;
  let counted = 0;
  for (const message of messages) {
...
export function buildOpencodeCompletionEvidence(
  goalObjective: string,
  messages: OpencodeMessage[],
  cwd: string,
): GoalDecisionEvidence {
...
function textFromMessageContent(message: OpencodeMessage): string | undefined {
  ...
  if (chunks.length === 0 && typeof message.content === "string") return message.content;
  return chunks.length > 0 ? chunks.join("\n").trim() : undefined;
}

function extractShellCommandFromPart(part: OpencodeMessagePart): string | undefined {
...
function extractPathFromPart(part: OpencodeMessagePart): string | undefined {
  const args = (part as { args?: Record<string, unknown> }).args;
  if (args && typeof args.path === "string") return args.path;
  if (args && typeof args.filePath === "string") return args.filePath;
  if (args && typeof args.file === "string") return args.file;
  return undefined;
}
