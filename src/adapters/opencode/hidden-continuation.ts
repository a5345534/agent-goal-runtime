...
export async function startOpencodeHiddenGoalTurn(
  context: OpencodeHiddenContinuationContext,
  request: HiddenGoalTurnRequest,
  registry: OpencodeHiddenContinuationRegistry,
): Promise<HiddenGoalTurnResult> {
  ...
  if (context.busy?.()) return { kind: "skipped", reason: "active turn is running" };
  if (context.hasQueuedUserInput?.()) return { kind: "skipped", reason: "user input is queued" };
...
function renderOpencodeGoalContinuationMessage(request: HiddenGoalTurnRequest): string {
  return [
...
export function extractOpencodeGoalContinuationMetadata(text: string): OpencodeGoalContinuationMetadata | undefined {
...
export function isOpencodeSessionIdleEvent(event: OpencodePluginEvent): boolean {
  return event?.type === "session.idle";
}

export function isOpencodeSessionErrorEvent(event: OpencodePluginEvent): boolean {
  return event?.type === "session.error";
}

export function isOpencodeSessionCompactedEvent(event: OpencodePluginEvent): boolean {
  return event?.type === "session.compacted";
}

export function extractOpencodeEventSessionID(event: OpencodePluginEvent): string | undefined {
...
export function rewriteOpencodeQueuedContinuations(
  messages: Array<{ info: Record<string, unknown>; parts: Array<Record<string, unknown>> }>,
  isCurrent: (metadata: OpencodeGoalContinuationMetadata) => boolean,
  currentGoalId?: string,
): { messages: Array<{ info: Record<string, unknown>; parts: Array<Record<string, unknown>> }>; changed: boolean } {
  const metadataByIndex = new Map<number, OpencodeGoalContinuationMetadata>();
  ...
  messages.forEach((message, index) => {
    for (const part of message.parts) {
      ...
      if (isCurrent(metadata)) currentIndices.push(index);
      break;
    }
  });
...
function staleOpencodeContinuationMessage(
  message: { info: Record<string, unknown>; parts: Array<Record<string, unknown>> },
  metadata: OpencodeGoalContinuationMetadata,
...
function supersededOpencodeContinuationMessage(
  message: { info: Record<string, unknown>; parts: Array<Record<string, unknown>> },
  metadata: OpencodeGoalContinuationMetadata,
): { info: Record<string, unknown>; parts: Array<Record<string, unknown>> } {
  return {
