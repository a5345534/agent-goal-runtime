...
class RpcClient {
  ...
  private stderrTail = "";
  private readonly pending = new Map<string, { resolve: (value: RpcResponse) => void; reject: (error: Error) => void; timer: ReturnType<typeof setTimeout> }>();
  ...
  request(command: string, params: Record<string, unknown> = {}, timeoutMs = 60_000): Promise<RpcResponse> {
  ...
  private handleLine(line: string): void {
    ...
    if (parsed.type === "response" && typeof parsed.id === "string") {
      ...
      if (parsed.success === false) pending.reject(new Error(parsed.error ?? `Pi RPC command '${parsed.command ?? parsed.id}' failed`));
  ...
  private captureStderr(chunk: string): void {
    ...
    if (chunk.trim()) log(`stderr: ${chunk.trim()}`);
  }
...
function isTerminalGoalStateEvent(event: Record<string, unknown>): boolean {
  ...
  if (data.kind !== "goal_snapshot") return false;
...
async function waitForCommand(): Promise<{ sessionName?: string; prompt: string }> {
  for (;;) {
    try {
      const parsed = JSON.parse(fs.readFileSync(config.commandPath, "utf8")) as Record<string, unknown>;
...
function shutdown(client: RpcClient): void {
  client.stop();
...
async function main(): Promise<void> {
  ...
  if (!sessionFile) throw new Error("Pi RPC child did not report a session file");
