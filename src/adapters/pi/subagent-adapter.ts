...
function parsePiSessionFileFromDisk(path: string): ParsedPiSessionState {
  const parsed: ParsedPiSessionState = { entryCount: 0, messageCount: 0 };
  ...
  try {
    ...
    carry += decoder.end();
    if (carry.trim()) parsePiSessionLine(carry, parsed);
  } finally {
    closeSync(fd);
  }
  return parsed;
...
function parsePiSessionLine(rawLine: string, parsed: ParsedPiSessionState): void {
  ...
  parsed.entryCount += 1;
  if (isRuntimeStateMirrorEntry(entry)) return;
...
function terminalishAssistantTextWithoutMarker(text: string | undefined): string | undefined {
  const cleaned = cleanupMarkerText(text);
  if (!cleaned) return undefined;
  if (RESULT_MARKER.test(cleaned) || BLOCKED_MARKER.test(cleaned) || STATUS_BLOCKED_MARKER.test(cleaned)) return undefined;
...
function currentAttemptCutoffAt(subagent: GoalSubagentRecord): string | undefined {
  ...
  if (cursorAt || subagent.attemptStartedAt) return cursorAt ?? subagent.attemptStartedAt;
