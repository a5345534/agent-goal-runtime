...
class ControllerActionTimeoutError extends Error {
  readonly actionKind: GoalControllerActionAttemptRecord["actionKind"];

  constructor(
    message: string,
    readonly actionAttempt: GoalControllerActionAttemptRecord,
  ) {
    super(message);
...
function controllerEventCategory(event: string): GoalControllerTypedEventCategory {
  if (event.startsWith("poll.")) return "poll";
  if (event.startsWith("stale") || event.includes(".stale") || event.includes("staleReplay")) return "node.staleDetected";
  if (event === "exception.decision" || event === "recovery.decision") return "recovery.decision";
  if (event.startsWith("recovery.rule") || event.includes("Rule")) return "recovery.rule";
  if (event.startsWith("recovery.") || event.startsWith("followup.")) return "recovery.action";
...
async function reconcileSubagentOutcomes(
  ...
  for (const subagent of latestSubagentPerNode(state.subagents)) {
    ...
    if (subagent.status === "failed") {
      ...
      if (restartedInterruptedReplacement) continue;
...
async function reconcileStaleControllerStates(
  runtime: GoalControllerRuntimePort,
  goalId: string,
  options: GoalControllerTickOptions,
  result: GoalControllerTickResult,
  ...
  for (const subagent of latestSubagents) {
    ...
    if (!node) continue;
    if (node.status !== "controllerValidating" && subagent.status !== "controllerValidating") continue;
    ...
    const observation: GoalAdapterObservationRecord = {
...
function controllerStateAgeMs(node: GoalDagNode, subagent: GoalSubagentRecord, nowIso: string): number {
  const nowMs = Date.parse(nowIso);
  const timestamps = [node.updatedAt, subagent.updatedAt]
    .map((value) => Date.parse(value))
    .filter((value) => Number.isFinite(value));
  if (!Number.isFinite(nowMs) || timestamps.length === 0) return 0;
  return Math.max(0, nowMs - Math.max(...timestamps));
}

async function reconcileStaleRunnerStartingNodes(
  runtime: GoalControllerRuntimePort,
  goalId: string,
  options: GoalControllerTickOptions,
  result: GoalControllerTickResult,
  ...
  for (const node of state.nodes) {
    ...
    if (!runnerStarting && !runnerPreparing && !retryableBlockedRunnerStart && !retryableInitialAllocationBlock) continue;
    if (state.subagents.some((subagent) => subagent.nodeId === node.nodeId)) continue;
    ...
    let allocation: GoalControllerWorkspaceAllocation | undefined;
...
function runnerStartingStateAgeMs(node: GoalDagNode, nowIso: string): number {
  const nowMs = Date.parse(nowIso);
...
function isRetryableStaleRunnerStartingBlock(node: GoalDagNode): boolean {
  const summary = node.lastValidationSummary ?? "";
  return node.status === "blocked" && (
    /stale runnerStarting restart failed: Background goal session cwd does not exist/i.test(summary) ||
    /workspace allocation failed while recovering stale (?:resourcesCreating|resourcesReady) state/i.test(summary)
  );
}

function isRetryableInitialWorkspaceAllocationBlock(node: GoalDagNode): boolean {
  const summary = node.lastValidationSummary ?? "";
  return node.status === "blocked" &&
...
function recoveryLoopSignature(adapterId: string, observation: GoalAdapterObservationRecord, decision: GoalRecoveryDecisionRecord): string {
  return [adapterId, observation.kind, normalizeExceptionSignature(observation), decision.action].join(":");
}

function countMatchingRecoveryLoopDecisions(state: GoalOrchestrationState, nodeId: string, signature: string): number {
...
function shouldOpenRecoveryCircuitBreaker(decision: GoalRecoveryDecisionRecord, previousLoopCount: number, loopLimit: number): boolean {
  if (!Number.isFinite(loopLimit) || loopLimit <= 0) return false;
  if (["markNodeBlocked", "askUser", "proposeRecoveryRule", "delegateToLegacyRecovery"].includes(decision.action)) return false;
...
async function tryRetryBlockedIntegration(
  runtime: GoalControllerRuntimePort,
  options: GoalControllerTickOptions,
  state: GoalOrchestrationState,
  node: GoalDagNode,
...
function latestControllerValidationPassSummary(subagent: GoalSubagentRecord): string | undefined {
  const results = subagent.controllerValidationResults ?? [];
  for (let index = results.length - 1; index >= 0; index -= 1) {
    const result = results[index];
    if (/controller validation passed/i.test(result)) return result;
  }
  return undefined;
}

function ageSince(timestamp: string | undefined, nowIso: string): number {
  if (!timestamp) return Number.POSITIVE_INFINITY;
  const nowMs = Date.parse(nowIso);
  const timestampMs = Date.parse(timestamp);
  if (!Number.isFinite(nowMs) || !Number.isFinite(timestampMs)) return Number.POSITIVE_INFINITY;
...
function isValidationFollowupCappedSummary(summary: string): boolean {
...
async function tryStartValidationCappedReplacement(
  runtime: GoalControllerRuntimePort,
  options: GoalControllerTickOptions,
  state: GoalOrchestrationState,
  node: GoalDagNode,
  ...
  if (replacementCount >= maxRetries || !canRestartOnSameResources(node, subagent)) return false;
...
function validationCappedReplacementCount(state: GoalOrchestrationState, nodeId: string): number {
  return state.subagents.filter((subagent) =>
    subagent.nodeId === nodeId &&
    subagent.lastRecoveryDecision?.ruleId === "validation-followup-cap-replacement" &&
    /^same-resource recovery\b/.test(subagent.integrationStatus ?? ""),
  ).length;
}

async function tryRestartInterruptedValidationCappedReplacement(
  runtime: GoalControllerRuntimePort,
  options: GoalControllerTickOptions,
  state: GoalOrchestrationState,
  node: GoalDagNode,
  ...
  if (previousDecision?.ruleId !== "validation-followup-cap-replacement") return false;
  if (!canRestartOnSameResources(node, subagent)) return false;
...
async function degradePromptDispatchFailure(
  runtime: GoalControllerRuntimePort,
  node: GoalDagNode,
  subagent: GoalSubagentRecord,
  result: GoalControllerTickResult,
...
function controllerActionAttemptFromError(error: unknown): GoalControllerActionAttemptRecord | undefined {
  if (error instanceof ControllerActionTimeoutError) {
...
async function startGoalSubagentWithTimeout(
  runtime: GoalControllerRuntimePort,
  options: GoalControllerTickOptions,
  adapter: HarnessSubagentAdapter,
  node: GoalDagNode,
  startOptions: StartGoalSubagentOptions,
  tickStartedAt: string,
): Promise<GoalSubagentRecord> {
  ...
  try {
    ...
    const succeededAttempt: GoalControllerActionAttemptRecord = {
      ...actionAttempt,
      status: "succeeded",
    ...
    const failedAttempt: GoalControllerActionAttemptRecord = {
      ...actionAttempt,
...
async function sendGoalSubagentPromptWithTimeout(
  runtime: GoalControllerRuntimePort,
  options: GoalControllerTickOptions,
  adapter: HarnessSubagentAdapter,
  subagent: GoalSubagentRecord,
  ...
  try {
    ...
    const succeededAttempt: GoalControllerActionAttemptRecord = { ...actionAttempt, status: "succeeded" };
...
function buildControllerActionAttempt(
  actionKind: GoalControllerActionAttemptRecord["actionKind"],
  target: Pick<GoalSubagentRecord, "goalId" | "nodeId" | "subagentId">,
  startedAt: string,
  timeoutMs: number | undefined,
  evidence: Record<string, unknown> = {},
): GoalControllerActionAttemptRecord {
...
async function promiseWithTimeout<T>(promise: Promise<T>, timeoutMs: number, message: string, actionAttempt: GoalControllerActionAttemptRecord): Promise<T> {
