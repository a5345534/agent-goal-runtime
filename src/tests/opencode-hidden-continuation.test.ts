...
function makeClient(record: { calls: Array<{ sessionID: string; text: string }> }) {
  return {
    session: {
      prompt: async ({ sessionID, parts }: { sessionID: string; parts?: Array<{ text?: string }> }) => {
...
const fixture = {
  ...
  attemptCount: 1,
  hiddenContextKind: "goal_continuation" as const,
...
test("startOpencodeHiddenGoalTurn sends a continuation text part and remembers the attempt", async () => {
  const record = { calls: [] as Array<{ sessionID: string; text: string }> };
...
test("startOpencodeHiddenGoalTurn skips when the session is busy", async () => {
  ...
  const client = makeClient(record);
  const registry = new OpencodeHiddenContinuationRegistry();
  const result = await startOpencodeHiddenGoalTurn(
    { client, sessionID: "ses_1", busy: () => true },
    fixture,
    registry,
  );
...
test("startOpencodeHiddenGoalTurn skips when user input is queued", async () => {
  ...
  const registry = new OpencodeHiddenContinuationRegistry();
  const result = await startOpencodeHiddenGoalTurn(
    { client, sessionID: "ses_1", hasQueuedUserInput: () => true },
    fixture,
    registry,
  );
  assert.equal(result.kind, "skipped");
...
test("rewriteOpencodeQueuedContinuations marks stale messages but keeps the latest current", () => {
  ...
  const isCurrent = (m: OpencodeGoalContinuationMetadata) => m.goalId === "goal-current";
  const result = rewriteOpencodeQueuedContinuations(messages, isCurrent, "goal-current");
  assert.equal(result.changed, true);
  // First is stale (not current)
  assert.match((result.messages[0]?.parts[0]?.text as string) ?? "", /Stale hidden goal continuation/);
  // Second is superseded (current but not latest)
  assert.match((result.messages[1]?.parts[0]?.text as string) ?? "", /Superseded hidden goal continuation/);
  // Third is kept verbatim (latest current)
  assert.match((result.messages[2]?.parts[0]?.text as string) ?? "", /latest/);
});
...
function messageWithPart(text: string) {
  return {
    info: {},
