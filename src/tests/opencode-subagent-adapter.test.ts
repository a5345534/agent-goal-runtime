...
function fakeLauncher() {
  ...
  const launcher = async (request: OpencodeBackgroundSessionLaunchRequest): Promise<OpencodeBackgroundSessionHandle> => {
...
test("Opencode subagent session inspection maps markers to self-report states", async () => {
  const blocked = readOpencodeSubagentSessionState(
    subagent(),
    { messages: [
      {
        id: "m1",
  ...
  const completed = readOpencodeSubagentSessionState(
    subagent(),
    { messages: [
      {
        id: "m1",
