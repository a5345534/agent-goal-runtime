...
import {
  isOpencodeCompletionAuditEnabled,
  opencodeHeuristicCompletionAudit,
} from "../adapters/opencode/completion-audit.js";
...
import type { CompletionAuditRequest, GoalRecord } from "../core/index.js";
...
function makeRequest(evidence: { commands?: string[]; verificationSignals?: string[]; toolNames?: string[] } = {}): CompletionAuditRequest {
  return {
    goal: makeGoal(),
    ledgerEvents: [],
    completionEvidence: {
...
test("isOpencodeCompletionAuditEnabled defaults to heuristic and respects off switch", () => {
  ...
  try {
    ...
    if (previous === undefined) delete process.env.AGENT_GOAL_COMPLETION_AUDIT;
    else process.env.AGENT_GOAL_COMPLETION_AUDIT = previous;
  }
});
