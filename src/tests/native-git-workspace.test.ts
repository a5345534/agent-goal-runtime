...
test("native git cleanup policy can remove blocked worktrees when explicitly requested", () => {
  ...
  try {
    ...
    const [result] = cleanupTerminalSubagentWorkspaces(
      manager,
      { goalId: "goal-abcdef12", nodes: [], subagents: [subagent] },
      { blocked: "remove", force: true },
    );
...
test("native git branch integrator plugs into controller completion gate", async () => {
  ...
  try {
    ...
    await runtime.saveGoalDagNode({ ...(await runtime.getGoalDagNode("goal-abcdef12", "build") as NonNullable<Awaited<ReturnType<GoalRuntime["getGoalDagNode"]>>>), status: "running", updatedAt: "2026-06-02T00:00:00.000Z" });
