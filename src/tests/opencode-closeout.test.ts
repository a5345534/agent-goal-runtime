...
import { GoalRuntime, NativeGitWorkspaceManager, SQLiteGoalStore } from "../core/index.js";
import { finalizeOpencodeGoalFromDagTerminalState, formatOpencodeCloseoutDiagnostics } from "../adapters/opencode/closeout.js";
import type { GoalDagNode, GoalRecord, GoalSubagentRecord, NativeGitSubagentCleanupResult } from "../core/index.js";
import type { ResolvedWorkspaceBinding } from "../adapters/opencode/workspace.js";

const NOW = new Date("2026-06-03T01:00:00.000Z");

function makeGoal(goalId: string): GoalRecord {
  return {
...
function makeNode(goalId: string, nodeId: string, status: GoalDagNode["status"]): GoalDagNode {
  return {
    goalId,
    nodeId,
    slug: nodeId,
...
function makeSubagent(goalId: string, nodeId: string, status: GoalSubagentRecord["status"], workspacePath: string | undefined): GoalSubagentRecord {
  return {
    goalId,
    nodeId,
...
test("formatOpencodeCloseoutDiagnostics surfaces cleanup errors", () => {
  const result: Parameters<typeof formatOpencodeCloseoutDiagnostics>[0] = {
    terminal: true,
    finalizationChanged: true,
    cleanup: [
      { action: "error", subagentId: "sa-1", workspacePath: "/x", branch: "feat/x", error: "permission denied" } as NativeGitSubagentCleanupResult,
    ],
