...
import {
  opencodeGoalPlugin,
  createOpencodeGoalPluginContext,
  setOpencodeClientForTests,
...
import type { OpencodePluginInput } from "../adapters/opencode/shims.js";
import type { OpencodeMessage } from "../adapters/opencode/session-transcript.js";
...
function makeMessage(id: string, text: string, role: "user" | "assistant" = "assistant"): OpencodeMessage {
  return {
    id,
    role,
...
function makeClient(messages: OpencodeMessage[] = []): OpencodeClient & { calls: Array<{ sessionID: string; parts: Array<{ type?: string; text?: string }> }> } {
  const calls: Array<{ sessionID: string; parts: Array<{ type?: string; text?: string }> }> = [];
  return {
    calls,
    session: {
      create: async () => ({ data: { id: "ses_new" } }),
      get: async ({ sessionID }) => ({ data: { id: sessionID, directory: process.cwd() } }),
      prompt: async ({ sessionID, parts }: { sessionID: string; parts?: Array<{ type?: string; text?: string }> }) => {
...
function makePluginInput(workspace: string, client: OpencodeClient): OpencodePluginInput {
  return {
    client,
    ...
    $: (() => Promise.resolve({ stdout: "", stderr: "", exitCode: 0 })) as OpencodePluginInput["$"],
  };
}
...
test("opencode adapter goal_command tool starts an orchestrated goal with a workspace", async () => {
  ...
  const client = makeClient();
  setOpencodeClientForTests(client);
  const launches: OpencodeBackgroundSessionLaunchRequest[] = [];
  setOpencodeBackgroundSessionLauncherForTests(async (request) => {
    ...
    return {
      ...
      stop: () => undefined,
    } satisfies OpencodeBackgroundSessionHandle;
  });
  try {
    ...
    const tool = hooks.tool?.goal_command as { execute: (args: { command: string }, ctx: { sessionID: string }) => Promise<string> };
...
test("opencode adapter get_goal tool returns the current goal summary", async () => {
  ...
  try {
    ...
    store.close?.();
    const tool = hooks.tool?.get_goal as { execute: (args: unknown, ctx: { sessionID: string }) => Promise<string> };
...
test("opencode adapter update_goal tool reports incomplete blocked evidence when only one turn is available", async () => {
  ...
  try {
    ...
    store.close?.();
    const tool = hooks.tool?.update_goal as { execute: (args: { status: string }, ctx: { sessionID: string }) => Promise<string> };
