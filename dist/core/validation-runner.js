...
function expectedOutputExists(request, output, changedBasenames) {
    const cwd = request.subagent.workspacePath;
    const path = isAbsolute(output) ? output : cwd ? resolve(cwd, output) : output;
    if (existsSync(path))
        return true;
    if (!cwd || isPathLikeOutput(output))
        return false;
    if (changedBasenames.has(output))
        return true;
    return workspaceBasenameMatchCount(cwd, output, 2) === 1;
}
function isPathLikeOutput(output) {
    return output.includes("/") || output.includes("\\");
}
function workspaceBasenameMatchCount(root, expectedBasename, limit) {
    let count = 0;
    const visit = (dir) => {
        let entries;
        try {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (shouldSkipExpectedOutputScanDir(entry.name))
                    continue;
                visit(resolve(dir, entry.name));
                continue;
            }
            if (entry.isFile() && entry.name === expectedBasename)
                count += 1;
        }
    };
    visit(root);
...
function shouldSkipExpectedOutputScanDir(name) {
...
function checkArtifactLocks(request) {
...
function highRiskValidationPolicyFailures(request) {
...
function evaluateRequiredEvidence(request, result) {
...
function isEvidenceSatisfied(requirement, request, result) {
    switch (requirement) {
        case "validators-ran":
...
function buildValidationSignals(result) {
    ...
    for (const evidence of result.satisfiedEvidence)
        signals.push(`satisfied evidence: ${evidence}`);
    for (const evidence of result.missingEvidence)
        signals.push(`missing evidence: ${evidence}`);
    for (const failure of result.policyFailures)
        signals.push(`policy failure: ${failure}`);
    if (signals.length === 0)
        signals.push("self-report accepted; no expected outputs, executable validators, artifact locks, or required evidence configured");
    return signals;
}
