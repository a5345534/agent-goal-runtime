...
export class GoalRuntime {
    ...
    async executeCommand(sessionKey, args, options = {}) {
        const command = parseGoalCommand(args);
        return this.executeParsedCommand(sessionKey, command, options);
    }
    async executeParsedCommand(sessionKey, command, options = {}) {
        switch (command.kind) {
            case "show":
    ...
    async finalizeGoalFromDagTerminalState(goalId) {
        ...
        const nextStatus = allComplete && integrationIssues.length === 0 ? "complete" : "blocked";
    ...
    async blockGoalFromControllerCloseout(goalId, reason, details = {}) {
        const goal = await this.getGoalById(goalId);
        if (!goal)
            return { goalId, terminal: true, changed: false, reason: "goal record not found" };
        if (goal.status !== "active") {
    ...
    async planGoalDagFromFileDocument(goalId, document, options = {}) {
    ...
    async startGoalSubagent(adapter, node, options) {
        ...
        const runningNode = withGoalDagNodeLifecyclePhase(attachPreparedResourcesToNode(node, preparedResources, { phase: "runnerActive", now: record.updatedAt }), "runnerActive", { status: "running", now: record.updatedAt });
    ...
    async resolveGoalReference(reference) {
        ...
        if (matches.length === 1)
            return { kind: "found", goal: matches[0] };
        if (matches.length > 1)
            return { kind: "ambiguous", reference: trimmed, matches };
    ...
    getCurrentTurnStop(sessionKey) {
        const stop = this.activeTurns.get(sessionKey)?.stopped;
        return stop ? { ...stop } : undefined;
    }
    async createOrReplaceGoal(sessionKey, objectiveInput, options = {}) {
    ...
    async toolGetGoal(sessionKey) {
        return this.getGoal(sessionKey);
    }
    async toolCreateGoal(sessionKey, objective, tokenBudget) {
        const existing = await this.store.getCurrentGoal(sessionKey);
        if (existing)
    ...
    async toolUpdateGoal(sessionKey, statusInput, options = {}) {
        ...
        if (status === "blocked") {
            if (goal.goalTurnsSinceAuditReset < this.config.blockedTurnsThreshold) {
                throw new Error(`blocked requires the same blocker for at least ${this.config.blockedTurnsThreshold} consecutive goal turns`);
            }
            ...
            if (evidence && evidence.consecutiveMatchingTurns < this.config.blockedTurnsThreshold) {
                ...
                throw new Error(`blocked requires matching blocker evidence for at least ${this.config.blockedTurnsThreshold} consecutive goal turns${detail}`);
            }
            const updated = await this.setGoalStatus(goal, "blocked");
        ...
        if (orchestrationState.nodes.length > 0) {
            ...
            if (unsuccessfulTerminal.length > 0) {
                throw new Error(`Goal cannot be completed: ${unsuccessfulTerminal.length} DAG node(s) are blocked or failed ` +
            ...
            if (integrationIssues.length > 0) {
                throw new Error(`Goal cannot be completed: ${integrationIssues.length} required subagent integration(s) incomplete ` +
    ...
    async usageLimitReached(sessionKey) {
        const goal = await this.requireGoal(sessionKey);
        await this.accountUsage(sessionKey);
    ...
    async sessionResumed(sessionKey) {
    ...
    async maybeContinueIfIdle(sessionKey) {
        if (!this.callbacks.startHiddenGoalTurn)
            return { kind: "notEligible", reason: "no hidden-turn callback" };
        await this.store.clearExpiredReservations(this.config.now());
        const existingReservation = await this.store.getReservation(sessionKey);
        if (existingReservation)
            return { kind: "notEligible", reason: "continuation already reserved" };
        const goal = await this.store.getCurrentGoal(sessionKey);
        if (!goal)
            return { kind: "notEligible", reason: "no current goal" };
        if (!isAutoContinuableStatus(goal.status))
            return { kind: "notEligible", reason: `goal is ${goal.status}` };
        const harnessState = await this.readHarnessState(sessionKey);
        const ineligible = explainHarnessIneligible(harnessState);
        if (ineligible)
            return { kind: "notEligible", reason: ineligible };
        ...
        let lastResult = { kind: "fatalFailure", error: "not attempted" };
    ...
    async requireGoal(sessionKey) {
        const goal = await this.store.getCurrentGoal(sessionKey);
        if (!goal)
            throw new Error("no current goal");
        return goal;
    }
    async setGoalStatus(goal, status) {
        const updated = { ...goal, status, updatedAt: this.nowIso() };
        await this.store.saveGoal(updated);
        await this.callbacks.notifyGoalUpdated?.(updated);
        return updated;
    }
    ...
    async readHarnessState(sessionKey) {
        ...
        return ((await this.callbacks.readHarnessState?.(sessionKey)) ?? {
            materialized: true,
            queuedUserInput: false,
            queuedTriggerTurn: false,
            continuationSuppressed: false,
    ...
    async runCompletionAuditIfConfigured(goal) {
        await this.appendLedger("completion_requested", goal.sessionKey, goal.goalId);
        if (!this.callbacks.auditCompletion)
            return undefined;
    ...
    markMeaningfulProgress(sessionKey) {
        const turn = this.activeTurns.get(sessionKey);
        if (turn)
            turn.madeMeaningfulProgress = true;
    }
    markTurnStopped(sessionKey, reason, goalId, message) {
        const turn = this.activeTurns.get(sessionKey);
        if (!turn)
            return;
        turn.stopped = {
    ...
    async appendLedger(type, sessionKey, goalId, details, date = this.config.now()) {
...
function explainHarnessIneligible(state) {
    if (!state.materialized)
        return "session is not materialized";
    if (state.activeTurnId)
        return "active turn is running";
    if (state.queuedUserInput)
        return "user input is queued";
    if (state.queuedTriggerTurn)
        return "trigger turn is queued";
    if (state.continuationSuppressed)
        return "continuation is suppressed";
    return undefined;
}
function backoffMs(base, jitter, attempt) {
