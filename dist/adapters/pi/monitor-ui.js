...
export class GoalMonitorController {
    ...
    render(width, theme) {
        ...
        if (view.liveFollowsTail && this.followLiveTail)
    ...
    buildView(dag, controllerTranscript) {
        ...
        if (this.listIndex === 0 || this.listIndex >= nodeSubagents.length)
            this.listIndex = selectedIndex;
...
function renderNodeLiveLines(node, subagents, transcript, now) {
    const latest = latestSubagent(subagents);
    const lines = [
