...
export class GoalMonitorController {
    ...
    get actions() {
        ...
        if (this.goal.status === "active")
            actions.push("pause");
        if (["active", "paused", "blocked", "budgetLimited", "usageLimited"].includes(this.goal.status))
            actions.push("resume");
        actions.push("clear");
    ...
    render(width, theme) {
        ...
        if (this.followTail)
            this.transcriptScroll = Math.max(0, transcriptLines.length - visibleTranscriptCount);
        else
            this.transcriptScroll = clampScroll(this.transcriptScroll, transcriptLines.length, visibleTranscriptCount);
        const lines = [
        ...
        for (const line of dagLines.slice(dagStart, dagEnd))
            lines.push(truncateToWidth(line, width));
        if (dagLines.length > 0)
            lines.push(truncateToWidth(theme.fg("dim", formatDagRange(dagStart, dagEnd, dagLines.length, this.activePane === "dag")), width));
...
function formatDagRange(start, end, total, active) {
    const details = [
...
function formatTranscriptRange(start, end, total, active, followTail) {
...
function renderDagLines(snapshot, now) {
    ...
    for (const [index, node] of snapshot.nodes.entries()) {
        ...
        if (node.modelScenario || node.modelArg)
            lines.push(`   model: ${formatMonitorModel(node.modelScenario, node.modelArg)}`);
...
function formatMonitorModel(scenario, model) {
    if (scenario && model)
